...
class BookUpdateBatchJobIntegrationTest : IntegrationTestSupport() {
    ...
    fun `SQSの1メッセージをS3のJSONLとして読み既存の書籍更新ユースケースで更新する`() {
        ...
        s3Client.putObject(
            PutObjectRequest.builder().bucket(BUCKET).key(inputKey).build(),
            RequestBody.fromString("""{"bookId":"${books.bookId2}","publishStatus":"PUBLISHED","price":2200}"""),
        )
        sendMessage("""{"taskToken":"task-token","data":{"id":"$taskId","inputFilePath":"$inputPath","outputDirectoryPath":"$outputDirectory"}}""")
        ...
        val output = s3Client.getObjectAsBytes(
            GetObjectRequest.builder().bucket(BUCKET).key(outputKey(taskId)).build(),
        ).asString(StandardCharsets.UTF_8)
        assertEquals("true", objectMapper.readTree(output).get("updated").asText())
    }

    private fun job(taskNotifier: TaskNotifier) = RunBookUpdateBatchJobUseCase(
    ...
    class AwsInitializer : ApplicationContextInitializer<ConfigurableApplicationContext> {
        override fun initialize(applicationContext: ConfigurableApplicationContext) {
            SqsClient.builder().endpointOverride(URI.create(endpoint())).region(Region.of(region()))
                .credentialsProvider(StaticCredentialsProvider.create(AwsBasicCredentials.create("test", "test"))).build().use {
