...
class ApplicationConfiguration {
    ...
    @Bean fun registerAuthorUseCase(
        authorRepository: AuthorRepository,
        authorDomainService: AuthorDomainService,
    ) = RegisterAuthorUseCase(authorRepository, authorDomainService)

    @Bean fun registerAuthorInputPort(
        useCase: RegisterAuthorUseCase,
        transactionTemplate: TransactionTemplate,
    ): RegisterAuthorInputPort = TransactionalRegisterAuthorInputPort(useCase, transactionTemplate)

    @Bean fun registerBookUseCase(
        bookRepository: BookRepository,
        authorDomainService: AuthorDomainService,
    ) = RegisterBookUseCase(bookRepository, authorDomainService)

    @Bean fun registerBookInputPort(
        useCase: RegisterBookUseCase,
        transactionTemplate: TransactionTemplate,
    ): RegisterBookInputPort = TransactionalRegisterBookInputPort(useCase, transactionTemplate)

    @Bean fun updateBookUseCase(
        bookRepository: BookRepository,
        authorDomainService: AuthorDomainService,
    ) = UpdateBookUseCase(bookRepository, authorDomainService)

    @Bean fun updateBookInputPort(
        useCase: UpdateBookUseCase,
        transactionTemplate: TransactionTemplate,
    ): UpdateBookInputPort = TransactionalUpdateBookInputPort(useCase, transactionTemplate)
...
private class TransactionalRegisterAuthorInputPort(
    private val useCase: RegisterAuthorUseCase,
...
private class TransactionalRegisterBookInputPort(
...
private class TransactionalUpdateBookInputPort(
    private val useCase: UpdateBookUseCase,
    private val transactionTemplate: TransactionTemplate,
) : UpdateBookInputPort {
